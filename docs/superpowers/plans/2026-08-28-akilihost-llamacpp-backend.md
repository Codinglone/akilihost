...
- `scripts/ai-host-tunnel.sh` — add api-product-dev as default server
...
Run:
```bash
DISK=$(az vm show -g PRODUCT-DEV -n api-product-dev --query "storageProfile.osDisk.name" -o tsv)
echo "OS disk: $DISK"
az disk update -g PRODUCT-DEV -n "$DISK" --size-gb 256
```
...
Run:
```bash
ssh api-product-dev 'sudo growpart /dev/sda 1 && sudo resize2fs /dev/sda1 && df -h / | tail -1'
```
...
Run:
```bash
ssh api-product-dev 'sudo apt-get update -y && sudo DEBIAN_FRONTEND=noninteractive apt-get install -y cuda-toolkit-12-8 && echo "=== nvcc ===" && nvcc --version'
```
Expected: nvcc reports CUDA 12.8 (or similar)

If `nvcc` is not in PATH after install, add it:
```bash
ssh api-product-dev 'echo "export PATH=/usr/local/cuda/bin:\$PATH" >> ~/.bashrc && export PATH=/usr/local/cuda/bin:$PATH && nvcc --version'
```
...
Run:
```bash
ssh api-product-dev 'nvidia-smi | head -5 && echo "---" && nvcc --version | tail -2'
```
...
In `scripts/ai-host-tunnel.sh`, change the default server from `myserver` to `api-product-dev`:

```bash
SERVER="${AI_HOST_SERVER:-api-product-dev}"
```
...
Run:
```bash
scp bin/akilihost api-product-dev:~/akilihost
ssh api-product-dev 'sudo mv ~/akilihost /usr/local/bin/akilihost && sudo chmod +x /usr/local/bin/akilihost'
```
...
Run:
```bash
ssh api-product-dev 'akilihost init'
```
...
Run:
```bash
ssh api-product-dev 'akilihost serve Qwen3.8-27B'
```
...
Run:
```bash
ssh api-product-dev 'akilihost ps && nvidia-smi'
```
...
git commit -m "feat: update tunnel script default to api-product-host"
